...
// 	const now = new Date();
// 	const data = sinon.useFakeTimers(now.getDay()).now;
// 	const curretnDay = days[data];
// 	console.log(curretnDay);
// 	console.log(getDayFunc());
// 	it('should return  current day', () => {
// 		expect(getDayFunc()).to.be.equal(curretnDay);
// 	});
//
// });
...
// 	const middle = sinon.stub(Math, 'round').returns(3);
...
// 		expect(getRandomUsers(user, minNum, middle)).to.be.eql(firstMiddle);
// 	});
//
// 	it('should return  second middle of arr', () => {
// 		const maxNum = random.returns(0.9);
// 		expect(getRandomUsers(user, maxNum, middle)).to.be.eql(secondMiddle);
// 		sinon.restore(maxNum);
// 	});
//
// });
...
describe('test Product class', () => {
	...
	it('should set value', () => {
		const price = product.setPrice(value);
